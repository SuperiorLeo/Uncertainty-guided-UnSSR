...
from basicsr.models.archs.RtxMamba2 import RetinexMamba

# from architecture.resblock import resblock,conv_relu_res_relu_block
import glob
...
def get_reconstruction_gpu(input, model):
    ...
    with torch.no_grad():
        ...
        # 用于有采样倍数乘除后无法匹配的情况
        # var_output1,before, out = model(var_input[:,:,:-2,:])
        # var_output2,before, out = model(var_input[:,:,2:,:])
...
def compute_ssim(label,output):
    ...
    mssim = [
        structural_similarity(label[i, :, :], output[i, :, :], data_range=data_range, multidimension=multidimension)
        for i in range(label.shape[0])]
...
model_path = '/home/lengyihong/workspace/SSR/ours/SFormer2020/AAAI_compare/RetinexMamba/net_42epoch.pth'
result_path = '/home/lengyihong/workspace/SSR/ours/SFormer2020/AAAI_compare/RetinexMamba/RetinexMamba2/'
# model_path = '/home/lengyihong/workspace/SSR/ours/SFormer2020/AAAI_compare/HDNet/net_37epoch.pth'
# result_path = '/home/lengyihong/workspace/SSR/ours/SFormer2020/AAAI_compare/HDNet/HDNet/'
...
# model = HDNet(3,31)
# model = AWAN(3, 31, 100, 10) # 需要用patch
# model = MST_Plus_Plus(3, 31, 31, 3) # 可以不用patch
# model = reconnet() # 2021USSR
# model = SGN(3, 31, 64)
# model = AWAN(3, 31, 100, 10) #DRCR
model = RetinexMamba(stage=1,n_feat=40,num_blocks=[1,2,2])
...
for i in range(len(img_path_name)):
    ...
    # _,img_res, idx  = get_reconstruction_gpu(rgb,model)
