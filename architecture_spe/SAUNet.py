...
class CMFormer(nn.Module):
    ...
    def __init__(self,
        ...
        # Output Mapping
        self.mapping = nn.Sequential(nn.Conv2d(self.dim, self.dim, 3, 1, 1, bias=False)  # 3x3 conv
                                     )
...
class SAUNet2(nn.Module):
    ...
    def __init__(self,
                 ...
                 ffn_ratio=4,
                 with_cp=False):
        super(SAUNet2, self).__init__()
        self.in_dim = in_dim
        self.dim = dim
        self.num_iterations = num_iterations
        self.denoisers = nn.ModuleList([])
        self.inc = nn.Conv2d(self.in_dim, self.dim, 1, 1, 0, bias=True)
        for _ in range(num_iterations):
            self.denoisers.append(
                CMFormer(in_dim=self.dim,
                         dim=self.dim,
                         cdpr=cdpr,
                         bdpr=bdpr,
                         num_blocks=num_blocks,
...
if __name__ == "__main__":
    ...
    model = SAUNet2(in_dim=3, dim=31, num_iterations=5)
    ...
    '''
    1 Parameters number is 881826.0; Flops: 8.849211392
    5 Parameters number is 4.412819; Flops: 44.495945728 Parameters number is 4.41282; Flops: 177.991909376
    9 Parameters number is 7.942975; Flops: 80.087826432
    13 Parameters number is 11.473131; Flops: 115.679707136
    '''